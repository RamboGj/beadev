			...
			[INLINES.HYPERLINK]: (data: any, children: ReactNode) => {
				return (
					<Link
						target="_blank"
						href={data.data.uri || ""}
						className="font-Satoshi text-sm text-blue500 hover:opacity-85 lg:text-base hover:text-pink500 transition duration-500"
					>
						{children}
					</Link>
				);
	...
	return (
		<main className="bg-neutral100 flex flex-col items-start w-full pb-32">
			<div
				...
				className="w-full h-[320px] lg:h-[600px] bg-cover bg-center bg-no-repeat"
				style={{ backgroundImage: `url(${article.cover.url})` }}
			/>

			<div className="max-w-[1120px] mt-12 w-full mx-auto flex flex-col gap-y-4 px-4 lg:px-0">
